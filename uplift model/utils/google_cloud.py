...
class BigQuery():
    ...
    def upload_csv_to_bigquery(self,table,filename):
        ...
        filename = 'extracto.csv' # this is the file path to your csv
        dataset_id = splits[1]
        table_id = splits[2]
...
class Storage():
    ...
    def downlad_file(self,gs_source_path,destination_file):
        if((gs_source_path[:5] != "gs://") | (len(gs_source_path.split("/")) < 3)):
            raise("Error de formato del bucket en upload_file. Formato esperado: gs://bucket/algo")
    ...
    def list_blobs(self,
                   bucket_name:str = 'marketing-modelling', 
                   prefix:str ='MP/EARLY_LTV/MLB/model'):
        """ List blob on a bucket using a prefix filter. Wrapper for a
        client.list_blobs method
        bucket_name: bucket_name (eg. marketing-modelling)
        prefix: prefix (eg.MP/EARLY_LTV/MLB/model )"""
        ...
        list_bolobs = [blob.name for blob in self.gsClient.list_blobs(
